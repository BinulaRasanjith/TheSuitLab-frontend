...
const authSlice = createSlice({ // create the auth slice
	...
	extraReducers: (builder) => { // the extra reducers of the slice. These reducers will be generated by the createAsyncThunk function. They will be used to update the status and the user state of the slice.
		builder
			...
			// logoutAsync reducers
			.addCase(logoutAsync.pending, (state) => { // this reducer will be called when the logoutAsync thunk is dispatched. It will set the status state to loading.
				state.status = LOADING;
			})
			.addCase(logoutAsync.fulfilled, (state) => { // this reducer will be called when the logoutAsync thunk is fulfilled. It will set the status state to idle and reset the user state.
				state.status = IDLE;
